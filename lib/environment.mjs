...
import {deploymentRole} from './deployment.mjs';
...
export function validateEnvironment() {
  if(deploymentRole()!=='backend') throw new Error('Backend providers are forbidden on the frontend');
  if(process.env.NODE_ENV==='production' || process.env.RENDER==='true') {
    if(process.env.BLONTIX_ISOLATED_QA || process.env.TEST_DATABASE_URL || Object.keys(process.env).some(name=>name.startsWith('QA_')) || (process.env.DATABASE_MIGRATIONS_SCHEMA && process.env.DATABASE_MIGRATIONS_SCHEMA!=='drizzle')) throw new Error('Test configuration is forbidden in production');
    if(/tests[\\/]support|register-s3-emulator/.test(process.env.NODE_OPTIONS || '')) throw new Error('Test transport is forbidden in production');
  }
  ...
  if((process.env.NODE_ENV==='production' || process.env.RENDER==='true') && (/^\/(blontix_qa_|isolated)/.test(url.pathname) || /blontix_qa_|search_path\s*=\s*(test|qa|isolated)/i.test(url.searchParams.get('options') || ''))) throw new Error('Test database is forbidden in production');
