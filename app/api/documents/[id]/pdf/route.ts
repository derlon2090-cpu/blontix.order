...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const row = await env.DB.prepare(
      "SELECT pdf_key, pdf_sha256, document_reference FROM order_documents WHERE id = ?"
    ).bind(id).first<{ pdf_key: string; pdf_sha256: string; document_reference: string }>();
    ...
    return new Response(message === "AUTH_REQUIRED" ? "يلزم تسجيل الدخول." : "تعذر تحميل النسخة الأصلية.", { status: message === "AUTH_REQUIRED" ? 401 : 500, headers: { "Cache-Control": "no-store" } });
  }
}
